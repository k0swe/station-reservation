...
Ham radio clubs need a lightweight way to manage reservations for shared radio stations when
enterprise groupware is unavailable.
...
- If a member is changed from approved to denied, all of their upcoming reservations in that club
  are cancelled (not deleted) and remain in reservation history.
...
- Reservation timestamps are stored canonically in UTC and rendered in each user's browser-local
  timezone.
...
- Clubs use two roles in MVP (Club Admin and Member), with Club Admin promotion/demotion controls
  and a minimum of one Club Admin at all times.
- Notifications are skipped for MVP; users check the app directly, with future extensibility for
  notifications.
